...
DEFAULT_OUTPUT = "result-python.csv"
...
def parse_sbomqs_profile_ntia(stdout: str):
    ...
    # Tìm line có "SBOM Quality Score"
    line = None
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark SBOM (Syft, Trivy, Cdxgen) trên Python repos, chấm sbomqs --profile ntia."
    )
    parser.add_argument(
        "--repo-list",
        ...
        for idx, repo_url in enumerate(repos, start=1):
            ...
            if not repo_dir.exists():
                ...
                if not res_clone["ok"]:
                    ...
                    # vẫn ghi 3 dòng với gen_elapsed_sec rỗng
            ...
            for tool in ["syft", "trivy", "cdxgen"]:
                ...
                if res_gen["ok"] and sbom_path.is_file():
                    ...
                    if g is not None:
                        grade = g
                    # ưu tiên num_of_component từ sbomqs nếu có, nếu không dùng từ json
